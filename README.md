...
# 指定 context path（URL 前缀，不含首尾斜杠）
npm create g2rain-app@latest g2rain-cms-app -- --context-path cms
# 或简写 positional
npm create g2rain-app@latest g2rain-cms-app cms
```

未指定 `--context-path` 时，交互式创建会提示输入；默认值会从项目名推导（如 `g2rain-cms-app` → `cms`）。
...
A: CLI 会替换模板中的占位符（如 `{{PROJECT_NAME}}`、`{{CONTEXT_PATH}}`）。若需新增占位符，请修改本仓库 `src/index.ts` 中的 `replaceTemplatePlaceholders` 等逻辑。
